...
// Vendor HCI command to set MitM Flag
#define BT_HCI_OP_VS_SET_MITM_FLAG BT_OP(BT_OGF_VS, 0x0012)

struct bt_hci_cp_vs_set_mitm_flag {
    uint8_t mitm_flag;
} __packed;

// Vendor specific command to send control pdu
#define BT_HCI_VS_SEND_CTRL_PDU BT_OP(BT_OGF_VS, 0x0013)
...
// Vendor specific command to config controller to block
// specific PDUs
#define BT_HCI_VS_SET_BLOCKED_CTRL_PDU BT_OP(BT_OGF_VS, 0x0014)
struct bt_hci_cp_vs_set_blocked_ctrl_pdu {
    uint8_t conn_update_ind:1;
    uint8_t chan_map_ind:1;
    uint8_t terminate_ind:1;
    uint8_t enc_req:1;
    uint8_t enc_rsp:1;
    uint8_t start_enc_req:1;
    uint8_t start_enc_rsp:1;
    uint8_t unknown_rsp:1;
    uint8_t feature_req:1;
    uint8_t feature_rsp:1;
    uint8_t pause_enc_req:1;
    uint8_t pause_enc_rsp:1;
    uint8_t version_ind:1;
    uint8_t reject_ind:1;
    uint8_t slave_feature_req:1;
    uint8_t conn_param_req:1;
    uint8_t conn_param_rsp:1;
    uint8_t reject_ext_ind:1;
    uint8_t ping_req:1;
    uint8_t ping_rsp:1;
    uint8_t length_req:1;
    uint8_t length_rsp:1;
    uint8_t phy_req:1;
    uint8_t phy_rsp:1;
    uint8_t phy_upd_ind:1;
    uint8_t min_used_chan_ind:1;
} __packed;
